...
# 实例化模型、定义损失函数和优化器
input_size = 1
model = LSTMModel(input_size=input_size, hidden_layer_size=100, num_layers=2, dropout=0.2).to(device)
...
for iteration in range(num_iterations):
    ...
    for i in range(len(Y_train_noisy_np)):
        if np.abs(Y_train_noisy_np[i] - train_predict[i]) > correction_threshold:
            ...
            weights[i] = weights[i] * 0.9  # 逐步减少权重，鼓励模型对校正后的标签有更多信任
...
Y_train_noisy_np = scaler.inverse_transform(Y_train_noisy_np)  # Inverse transform noisy labels
Y_train = scaler.inverse_transform(Y_train.reshape(-1, 1))  # Inverse transform original labels
val_predict = scaler.inverse_transform(val_predict)
Y_val_noisy = scaler.inverse_transform(Y_val_noisy.cpu().numpy())  # Inverse transform noisy labels
...
val_mae = mean_absolute_error(Y_val_noisy, val_predict)
val_mse = mean_squared_error(Y_val_noisy, val_predict)
...
plt.legend()
plt.savefig('prediction_correctv3.png')
plt.show()
