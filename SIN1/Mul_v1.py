...
Y_val_noisy = inverse_transform(Y_val_noisy.cpu().numpy(), data, scaler)  # Inverse transform noisy labels
test_predict = inverse_transform(test_predict, data, scaler)
Y_test = inverse_transform(Y_test.cpu().numpy().reshape(-1, 1), data, scaler)
