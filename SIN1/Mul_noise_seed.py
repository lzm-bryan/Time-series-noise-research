...
def train_and_evaluate(seed):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'mbest_model.pth')

        if epoch % 10 == 0:
            print(f'Epoch {epoch} loss: {epoch_loss / len(train_dataloader)}, Validation loss: {val_loss}')

    # 加载最佳模型
    model.load_state_dict(torch.load('mbest_model.pth'))
    model.eval()
    with torch.no_grad():
    ...
    Y_test_inv = inverse_transform(Y_test_tensor.cpu().numpy(), data, scaler)
