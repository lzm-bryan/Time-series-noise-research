...
# 读取CSV数据
df = pd.read_csv('ETTm1.csv', index_col='date', parse_dates=True)
...
def train_and_evaluate(seed, params):
    ...
    model = EvidentialLSTMModel(input_size=1, hidden_layer_size=100, num_layers=2, dropout=0.2).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch_X, batch_Y in train_dataloader:
            ...
            loss = base_loss_function(output, batch_Y)
            loss.backward()
            optimizer.step()
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), 'edlbest_model.pth')

    # 校正标签并使用加权损失函数重新训练
    model.eval()
    with torch.no_grad():
        train_predict, uncertainty = model(X_train_tensor.to(device))
    ...
    for i in range(len(Y_train_noisy_np)):
        if uncertainty[i] > correction_threshold:
            weights[i] = 0.5
    ...
    for iteration in range(num_iterations):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0 or epoch == epochs - 1:
                ...
                val_loss /= len(val_dataloader)
                print(f'Iteration {iteration}, Epoch {epoch}: Train loss: {epoch_loss / len(train_dataloader_corrected)}, Validation loss: {val_loss}')

                if val_loss < best_val_loss:
                    best_val_loss = val_loss
                    torch.save(model.state_dict(), 'edlbest_model.pth')

        model.load_state_dict(torch.load('edlbest_model.pth'))
        model.eval()
        with torch.no_grad():
            train_predict, uncertainty = model(X_train_tensor.to(device))
            train_predict = train_predict.cpu().numpy()
    ...
    model.eval()
    with torch.no_grad():
        test_predict, _ = model(X_test_tensor.to(device))
...
# 保存并展示图表
plt.savefig('edl_metrics_comparison.png')
plt.show()
